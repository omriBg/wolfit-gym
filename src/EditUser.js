...
function EditUser({ onBackClick }) {
    const [selectedSports, setSelectedSports] = useState([]);
    const [preferenceMode, setPreferenceMode] = useState('simple');
    const [intensityLevel, setIntensityLevel] = useState(2); // 1, 2, 3
...
function getSportDisplay(sportId) {
  ...
  if (preferenceMode === 'simple') {
    ...
    const position = selectedSports.indexOf(sportId);
    return position + 1;
  }
}
